...
app.get('/api/patterns', (req, res) => {
  ...
  res.json({ patterns: listPatterns(type as any) });
});
...
app.post('/api/runtimes/execute', async (req, res) => {
  ...
  if (!code || !runtime) return res.status(400).json({ error: 'code and runtime required' });
  try {
...
app.get('/api/observability/metrics', (req, res) => {
  ...
  const points = queryMetrics(name as string, since ? Number(since) : undefined);
  res.json({ points });
});
...
app.get('/api/observability/events', (req, res) => {
  ...
  if (type && data) logEvent(String(type), { data: JSON.parse(String(data)) });
  res.json({ ok: true });
});
