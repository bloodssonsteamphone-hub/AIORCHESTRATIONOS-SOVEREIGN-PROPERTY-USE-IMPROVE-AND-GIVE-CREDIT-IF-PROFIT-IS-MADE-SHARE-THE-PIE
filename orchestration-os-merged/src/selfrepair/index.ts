...
const REPAIR_HANDLERS: Record<string, () => Promise<RepairResult>> = {
...
export async function runSelfRepair(): Promise<SelfRepairReport> {
...
function generateRecommendations(checks: CheckResult[], repairs: RepairResult[]): string[] {
  const recs: string[] = [];
  const failedRepairs = repairs.filter(r => !r.repaired);
  if (failedRepairs.length > 0) recs.push('Manual intervention required for failed repairs');
  const slowTools = checks.find(c => c.component === 'tool-failure' && c.status === 'fail');
  if (slowTools) recs.push('Consider restarting degraded tools or increasing timeout');
  const brainWarn = checks.find(c => c.component === 'brain-latency' && c.status === 'warn');
  if (brainWarn) recs.push('Brain latency elevated — monitor for continued degradation');
  if (checks.every(c => c.status === 'pass')) recs.push('System fully healthy — no action needed');
  return recs;
}

async function repairToolFailure(): Promise<RepairResult> {
  const issues: string[] = [];
...
async function repairLLMProvider(): Promise<RepairResult> {
  return {
...
async function repairBrainLatency(): Promise<RepairResult> {
  const brain = getPrimaryBrain();
...
async function repairMemoryPressure(): Promise<RepairResult> {
  const { getMemoryFabric } = await import('../brains/index.js');
  const fabric = getMemoryFabric();
...
async function repairWorkflowStuck(): Promise<RepairResult> {
