...
function execAsync(cmd: string, args: string[]): Promise<{ stdout: string; stderr: string; exitCode: number }> {
...
function callOllama(prompt: string, model: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', () => {
      try { const j = JSON.parse(data); resolve(j.response || ''); } catch { reject(new Error('Ollama parse error')); }
    });
...
export const imageGenTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    ...
    const { prompt, outputPath, width, height } = params as { prompt: string; outputPath?: string; width?: number; height?: number };
    try {
...
export const imageEditTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    const start = Date.now();
    const { action, inputPath, outputPath, width, height, filter } = params as {
      action: string; inputPath?: string; outputPath?: string; width?: number; height?: number; filter?: string;
    };
    if (!inputPath) throw new Error('inputPath required for image.edit');
    try {
      ...
      if (exitCode !== 0) throw new Error(stderr || 'ImageMagick convert failed');
...
export const audioSpeakTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    const start = Date.now();
    const { text, voice, outputPath, rate } = params as { text: string; voice?: string; outputPath?: string; rate?: number };
    if (!text) throw new Error('text required for audio.speak');
    try {
...
export const videoExtractFramesTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    const start = Date.now();
    const { action, videoPath, outputDir, fps, timestamp } = params as {
      action: string; videoPath?: string; outputDir?: string; fps?: number; timestamp?: string;
    };
    if (!videoPath) throw new Error('videoPath required for video.extract_frames');
    try {
      const outDir = outputDir || `/tmp/zo-frames-${Date.now()}`;
      const mkdirResult = await execAsync('mkdir', ['-p', outDir]);
      if (mkdirResult.exitCode !== 0) throw new Error('Could not create output directory');
      ...
      switch (action) {
        case 'extract-frame': {
          ...
          if (exitCode !== 0) throw new Error(stderr || 'ffmpeg frame extract failed');
        ...
        case 'extract-scene': {
          ...
          if (exitCode !== 0) throw new Error(stderr || 'ffmpeg scene extract failed');
