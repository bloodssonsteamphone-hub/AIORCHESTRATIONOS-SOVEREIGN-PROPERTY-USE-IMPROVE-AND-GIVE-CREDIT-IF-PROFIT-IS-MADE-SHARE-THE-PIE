...
import type { LLMResult, LLMTier } from '../types.js';

const fedClientPromise = createFederatedClient();
let fedClient: Awaited<ReturnType<typeof createFederatedClient>> | null = null;
...
export const llmTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    ...
    const { action, prompt, messages, system, model, tier, imageUrl } = params as {
      action: string; prompt?: string; messages?: Array<{role: string; content: string}>;
      system?: string; model?: string; tier?: LLMTier; imageUrl?: string;
    };
    const client = fedClient;
    try {
      switch (action) {
        case 'complete': {
          if (!prompt) throw new Error('prompt required for llm.complete');
          if (!client) throw new Error('Federated client not initialized yet');
          const result = await client.llm(prompt, { system, tier });
          return { success: true, data: result, duration: Date.now() - start };
        }
        case 'chat': {
          if (!messages?.length) throw new Error('messages required for llm.chat');
          // Convert chat messages to a single prompt (simple approach)
          if (!client) throw new Error('Federated client not initialized yet');
          const fullPrompt = messages.map((m: { role: string; content: string }) => `${m.role}: ${m.content}`).join('\n');
        ...
        case 'plan': {
          ...
          const result = await client.llm(
            `You are a planner. Create a structured, numbered plan for: ${planningPrompt}\n\nRespond with valid JSON only: {"steps": ["step 1", ...], "estimatedTime": "X minutes", "risks": ["risk 1", ...]}`,
            { system: 'You are a careful planner. Always respond valid JSON.', model, tier }
          );
          return { success: true, data: result, duration: Date.now() - start };
        }
        case 'vision': {
          if (!imageUrl) throw new Error('imageUrl required for llm.vision');
          if (!client) throw new Error('Federated client not initialized yet');
          const result = await client.llm(
            `Describe this image in detail: ${imageUrl}`,
            { model: 'qwen2-vl', tier: 'local-free' }
          );
          return { success: true, data: result, duration: Date.now() - start };
        }
        case 'embed': {
          ...
          if (!client) throw new Error('Federated client not initialized yet');
          const result = await client.llm(
            `Generate a dense vector embedding for: ${textToEmbed}\n\nRespond with a JSON array of floats.`,
            { model: 'nomic-embed-text', tier: 'local-free' }
          );
          return { success: true, data: result, duration: Date.now() - start };
        }
        default:
...
export const llmFederationTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    const start = Date.now();
    const { prompt, system, strategy, tiers, agents, maxParallel } = params as {
      prompt: string; system?: string;
      strategy?: 'consensus' | 'champion-challenger' | 'specialist-routing' | 'sequential';
      tiers?: LLMTier[]; agents?: string[]; maxParallel?: number;
    };
    try {
      let client = fedClient;
      if (!client) { client = await fedClientPromise; }
