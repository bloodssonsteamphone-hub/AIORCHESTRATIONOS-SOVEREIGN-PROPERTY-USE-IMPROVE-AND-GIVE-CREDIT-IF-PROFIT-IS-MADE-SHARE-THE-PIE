...
const sqliteDb: Record<string, Record<string, unknown>[]> = {};

function parseSQLite(query: string): { action: string; table?: string; data?: Record<string, unknown> } {
...
export const dbTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    ...
    const { action, connection, query, table, data, backupPath } = params as {
      action: string; connection?: string; query?: string; table?: string;
      data?: Record<string, unknown>; backupPath?: string;
    };
    try {
      switch (action) {
        case 'query': {
          if (!query) throw new Error('query required for db.query');
        ...
        case 'insert': {
          if (!table || !data) throw new Error('table and data required for insert');
          if (!sqliteDb[table]) sqliteDb[table] = [];
        ...
        case 'migrate': {
          ...
          tables.forEach(t => { if (!sqliteDb[t]) sqliteDb[t] = []; });
...
export const pgTool = {
  ...
  invoke: async (params: Record<string, unknown>) => {
    const start = Date.now();
    const { action, sql, backupPath } = params as { action: string; sql?: string; backupPath?: string };
