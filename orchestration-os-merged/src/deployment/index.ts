...
export interface DeploymentTarget {
  id: string;
  name: string;
  platform: 'windows' | 'android' | 'web' | 'linux' | 'macos';
  description: string;
  artifacts: string[];
  supported: boolean;
  checkCommand?: string;
...
export async function deploy(artifactPath: string, targetId: string, outputPath: string): Promise<DeploymentResult> {
  const start = Date.now();
  const target = TARGETS.find(t => t.id === targetId);
  if (!target) return { success: false, error: `Unknown target: ${targetId}`, duration: 0 };
...
function readdirDeep(dir: string, base = ''): string[] {
  try {
