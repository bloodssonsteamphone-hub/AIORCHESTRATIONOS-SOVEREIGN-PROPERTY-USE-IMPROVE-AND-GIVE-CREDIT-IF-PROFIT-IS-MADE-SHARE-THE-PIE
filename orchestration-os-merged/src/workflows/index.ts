// Layer 4 — Workflow Engine v3.1 (parallel step execution)
import type { Workflow, WorkflowStep, WorkflowStatus, WorkflowConfig, WorkflowMetrics, Runtime } from '../types.js';

export { createWorkflowEngine, WORKFLOW_CATALOG };

interface WorkflowEngine {
  run: (workflowId: string, context: Record<string, unknown>) => Promise<Workflow>;
  get: (workflowId: string) => Workflow | undefined;
  list: () => Workflow[];
  register: (workflow: Workflow) => void;
}

const WORKFLOW_CATALOG: Record<string, Omit<Workflow, 'status' | 'metrics'>> = {
...
function createWorkflowEngine(): WorkflowEngine {
  ...
  function getMetrics(wf: Workflow): WorkflowMetrics {
    return wf.metrics || { totalRuns: 0, successRate: 0, avgDuration: 0, avgStepsCompleted: 0 };
  }

  function cloneWorkflow(id: string): Workflow | undefined {
  ...
  async function run(workflowId: string, context: Record<string, unknown> = {}): Promise<Workflow> {
    let wf = workflows.get(workflowId);
    if (!wf) wf = cloneWorkflow(workflowId);
    ...
    if (wf.config.parallel) {
      ...
      for (const phase of phases) {
        const results = await Promise.allSettled(
          phase.map(step => executeStepAsync(step, ctx, stepMap, completed))
        );
        ...
        for (let i = 0; i < phase.length; i++) {
          ...
          if (result.status === 'fulfilled') {
            ...
            completed.add(step.id);
            allSucceeded = false;
            if (wf.config.rollbackOnFailure) break;
          }
        }
  ...
  function canRun(step: WorkflowStep, stepMap: Map<string, WorkflowStep>, completed: Set<string>): boolean {
  ...
  async function executeStep(step: WorkflowStep, _ctx: Record<string, unknown>): Promise<unknown> {
    ...
    switch (domain) {
      ...
      case 'run': {
        ...
        const cmd = step.params?.command as string;
  ...
  function list(): Workflow[] { return Array.from(workflows.values()); }
  function register(workflow: Workflow) { workflows.set(workflow.id, workflow); }

  for (const id of Object.keys(WORKFLOW_CATALOG)) cloneWorkflow(id);
