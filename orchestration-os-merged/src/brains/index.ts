// Intelligence Core — Primary Brain + Backup Brain + Memory Fabric
import type { Brain, BrainMetrics, BackupBrain, FailoverPolicy, MemoryLayer, MemoryFabric, BrainRole } from '../types.js';
...
function cleanExpired() {
  ...
  for (const [k, v] of store) {
    if (v.expiry && v.expiry < now) store.delete(k);
  }
}

// ─── Memory Layers ─────────────────────────────────────────────────────────────

function makeMemoryLayer(id: string, name: string, _backend: string, ttl = 0): MemoryLayer {
  return {
    id, name,
    ...
    read: async (key: string) => {
      ...
      if (entry.expiry && entry.expiry < Date.now()) { store.delete(`${id}:${key}`); return undefined; }
      return entry.value;
    },
    write: async (key: string, value: unknown, layerTtl?: number) => {
      const expiry = layerTtl || ttl ? Date.now() + (layerTtl || ttl || 86400000) : undefined;
      store.set(`${id}:${key}`, { value, expiry });
    },
    delete: async (key: string) => { store.delete(`${id}:${key}`); },
    list: async (pattern?: string) => {
      cleanExpired();
      const prefix = `${id}:`;
      const all = Array.from(store.keys()).filter(k => k.startsWith(prefix));
      if (!pattern) return all.map(k => k.replace(prefix, ''));
...
function createMemoryFabric(): MemoryFabric {
  ...
  return {
    ...
    async read(layer, key) {
      if (layer === 'short') return shortTerm.read(key);
      if (layer === 'mid') return midTerm.read(key);
      if (layer === 'long') return longTerm.read(key);
      throw new Error(`Unknown layer: ${layer}`);
    },
    async write(layer, key, value, ttl) {
      if (layer === 'short') return shortTerm.write(key, value, ttl);
      if (layer === 'mid') return midTerm.write(key, value, ttl);
      if (layer === 'long') return longTerm.write(key, value, ttl);
      throw new Error(`Unknown layer: ${layer}`);
    },
    async checkpoint(workflowId: string) {
    ...
    async resume(workflowId: string) {
      ...
      for (const [k, v] of Object.entries(data.store as Record<string, { value: unknown; expiry?: number }>)) {
        store.set(k, v);
      }
      return data;
    },
...
let primaryBrain: Brain;
let backupBrain: BackupBrain;
let memoryFabric: MemoryFabric;
...
function makeBrainMetrics(): BrainMetrics {
...
function createBrainSystem() {
  ...
  backupBrain = {
    ...
    roles: ['shadow-eval', 'hot-standby', 'failover'] as BrainRole[],
    metrics: makeBrainMetrics(),
  };
...
function getPrimaryBrain(): Brain { return primaryBrain; }
function getBackupBrain(): BackupBrain { return backupBrain; }
function getMemoryFabric(): MemoryFabric { return memoryFabric; }
...
async function makeDecision(context: { task: string; agents?: string[]; tools?: string[] }): Promise<{ routing: string; agents: string[]; tools: string[] }> {
  const start = Date.now();
  const task = context.task.toLowerCase();

  let tier = 'cloud-free';
  if (task.includes('simple') || task.includes('list') || task.includes('read')) tier = 'local-free';

  const agents: string[] = ['coder'];
  if (task.includes('architect') || task.includes('design')) agents.unshift('architect');
  if (task.includes('test')) agents.push('tester');
  if (task.includes('docs') || task.includes('document')) agents.push('docs');

  const tools: string[] = ['file', 'http'];
  if (task.includes('test')) tools.push('test-runner');
  if (task.includes('package') || task.includes('zip')) tools.push('zip-bundler');
  tools.push('metrics');
