...
let _workflowEngineRef = null;
export function setWorkflowEngineRef(ref) { _workflowEngineRef = ref; }
export const workflowTool = {
    ...
    invoke: async (params) => {
        ...
        const { action, workflowId, definition, params: wfParams } = params;
        const engine = _workflowEngineRef;
        try {
            switch (action) {
                case 'define': {
                    if (!definition)
                        throw new Error('definition required for workflow.define');
                ...
                case 'run': {
                    if (!workflowId)
                        throw new Error('workflowId required for workflow.run');
                    if (!engine)
                        return { success: false, error: 'Workflow engine not initialized', duration: Date.now() - start };
                    const result = await engine.runWorkflow(workflowId, wfParams);
                    return { success: true, data: result, duration: Date.now() - start };
                }
                case 'status': {
                    if (!workflowId)
                        throw new Error('workflowId required for workflow.status');
                    const session = engine?.sessions?.get?.(workflowId);
                    if (!session)
                        return { success: false, error: `Workflow ${workflowId} not found`, duration: Date.now() - start };
                ...
                case 'stop': {
                    if (!workflowId)
                        throw new Error('workflowId required for workflow.stop');
