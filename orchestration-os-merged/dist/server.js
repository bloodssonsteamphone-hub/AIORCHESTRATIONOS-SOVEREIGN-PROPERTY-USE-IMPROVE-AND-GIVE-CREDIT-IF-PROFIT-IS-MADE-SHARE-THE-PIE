...
function bootstrap() {
    ...
    try {
        ...
        console.log(`[Init] Tools warning: ${e.message}`);
    }
    // Init orchestrator
    try {
        initOrchestrator();
        ...
        console.log(`[Init] Orchestrator warning: ${e.message}`);
    }
    ...
    try {
        ...
        console.log(`[Init] Workflow engine warning: ${e.message}`);
    }
}
// ─── Health ───────────────────────────────────────────────────────────────────
...
app.get('/api/workflows', (req, res) => {
    ...
    res.json({ workflows: _workflowEngine.list().map((w) => ({ id: w.id, name: w.name, status: w.status })) });
});
...
app.post('/hooks/http', async (req, res) => {
    ...
    if (url) {
        ...
        res.json({ success: true, message: 'Hook received' });
    }
});
